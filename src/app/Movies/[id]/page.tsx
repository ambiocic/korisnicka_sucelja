...
export default function MoviePage({ params }: PostProps) {
  ...
  const handleAddToWatchlist = async () => {
    ...
    const { error } = await supabase.from("watchlist").insert([{ user_id: user.id, movie_id: media.id, tv_show_id: null }]);
    ...
    <main className="flex flex-col min-h-screen">
      <div className="flex-1 flex flex-col items-center p-6 mt-28 w-full">
        <div className="w-full max-w-4xl mb-4">
          <Link href="/Movies" className="inline-flex items-center text-yellow-400 font-semibold hover:text-yellow-500 transition-colors">
            ← Back to Movies
          </Link>
        </div>
