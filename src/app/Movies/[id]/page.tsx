...
export default function MoviePage({ params }: PostProps) {
  const [media, setMedia] = useState<Media | null>(null);
  const [reviews, setReviews] = useState<Review[]>([]);
  const [user, setUser] = useState<User | null>(null);
  const [newReview, setNewReview] = useState<{ rating: number; text: string }>({
    rating: 0,
    text: "",
  });
  ...
  const handleAddToWatchlist = async () => {
    ...
    const { error } = await supabase
      .from("watchlist")
      .insert([{ user_id: user.id, movie_id: media.id, tv_show_id: null }]);
    ...
    <main className="flex flex-col min-h-screen">
      <div className="flex-1 flex flex-col items-center p-6 mt-28 w-full">
        <div className="w-full max-w-4xl mb-4">
          <Link
            href="/Movies"
            className="inline-flex items-center text-yellow-400 font-semibold hover:text-yellow-500 transition-colors"
          >
            ← Back to Movies
          </Link>
        </div>
          ...
          {media.image && (
            <div className="flex flex-col items-center w-full md:w-1/3">
              ...
              <button
                ...
                className="bg-yellow-400 font-extrabold text-white py-2 px-4 rounded-lg hover:bg-yellow-500 w-full mt-2"
              >
                Add to Watchlist
              </button>
            </div>
        ...
        <div className="w-full max-w-4xl mt-8 flex flex-col items-baseline">
          {user && (
            <button
              onClick={toggleModal}
              className="mt-6 w-full md:w-auto bg-green-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 transition-colors"
            >
              Add a Review
            </button>
          )}
                ...
                >
                  <div className="flex justify-between items-center mb-2">
                    <strong className="text-gray-800 dark:text-gray-400">
                      {getDisplayName(review.email ?? undefined)}
                    </strong>
